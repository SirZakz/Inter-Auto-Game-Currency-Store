<x-content.shop-card color="{{ $article->color }}">
    ...
    <div class="pt-2 mt-auto">
            <button type="button" data-id="{{ $article->product_id }}" data-cost="{{ $article->costs }}" data-name="{{ $article->name }}" class="add-to-cart-button w-full rounded bg-green-600 hover:bg-green-700 text-white p-2 border-2 border-green-500 transition ease-in-out duration-150 font-semibold">
             {{ __('Add to Cart (RM:cost)', ['cost' => $article->costs]) }}
            </button>
    </div>
